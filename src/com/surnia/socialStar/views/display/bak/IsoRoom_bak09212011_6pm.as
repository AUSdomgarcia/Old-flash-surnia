...
*******************************************************************************************************************************************************/	
	public class IsoRoom extends MovieClip
	{
		...
		*********************************************************************************************************************************************/
		...
		private function addFlooring(_id:String):void 
		{
			...
			for (var i:int = 0; i < GRID_WIDTH; i++)
			{
				for (var j:int = 0; j < GRID_LENGTH; j++)
				{
					...
					_floors.addEventListener( MouseEvent.CLICK, onClickMyFloor );
		...
		private function onClickMyFloor(e:ProxyEvent):void 
		{
		...
		{			
			...
			*/	
			...
			//isoObject.instance.addEventListener( MouseEvent.MOUSE_DOWN, onInteractOfficeObject  );
		...
		* floor event handler
		...
		private function onObjectSelect( e:ProxyEvent ):void
		{
			...
			if (ALL_INTERACTIONS_ON) 
			{
					...
					if (OBJECT_COLLISSION == false && SPACE_COLLISSION == false) {
						if (OBJECT_MOVING) 
						{
							if (_activeObject == _dragObject) {
								...
								removePointer();
								updateOfficeObjectGridXY(e.target as IsoSprite);
								registerObject(e.target as IsoSprite , "move" );
								_activeObject = null;
							}
						}else {
				...
				-----------------------------*/
				case "ROTATE" :		
					trace( "rotate n please ==========================> pota!!!" );
					rotateOfficeObject( _dragObject );					
					removePointer();					
				break;
		...
		public function rotateOfficeObject( instance:IsoSprite ):void 
		{
			...
			{				
				if ( objectsAry[ i ].instance == instance ) {					
					...
					isoSprite.rotate();
					//updateOfficeObjectGridXY( isoSprite );
					//registerObject( isoSprite );
					_sdc.registerOfficeItemPosition(  isoSprite.entryid, isoSprite.gridX 
													 ,isoSprite.gridY, isoSprite.z
													 ,isoSprite.rotation,"rotate"
													);
					break;
				}
			}
		...
		private function registerObject( _target:IsoSprite, met:String ):void 
		{			
			//swifer
		...
		{
			_sdc.registerOfficeItemPosition( entryid, x,y,z,rot,"move" );
		}
		...
		{		
			...
			for (var j:int = 0; j < len; j++) 
			{
				if ( objectsAry[ j ].entryid == e.obj.entryid ) {
					...
					pathGrid.setWalkable( isoObject.gridX , isoObject.gridY , false);
		...
		private function putToTile(_target:IsoSprite):void 
		{
			for (var i:int = 0; i < floorsAry.length; i++ ) 
			{
				if( _target != null && floorsAry[i]!= null ){
					if (_target.x == floorsAry[i].x && _target.y == floorsAry[i].y) 
					{
						if (floorsAry[i].occupied == 0)
						{		
					...
					trace( "[isoroom]: putotile null detected!!" );
				}
			}
		...
		{
			//test new
			addEventListener( MouseEvent.CLICK, onDropOfficeItem );			
			...
			if (_dragObject != null && OBJECT_MOVING == true) {
				var pt:Pt = _view.localToIso( new Point( stage.mouseX, stage.mouseY + 15 ) );				
		...
		* main loop
		...
		public function clearObjectData():void
		{
			if( objectsAry != null ){
				...
				for (var j:int = 0; j < len; j++) 
				{
					if ( objectsAry[ j ] != null ){
						if ( _scene.contains( objectsAry[ j ].instance ) ){
							
							if (objectsAry[j].extraSpace != null)
							{
								if( _scene.contains( objectsAry[j].extraSpace ) ){
									_scene.removeChild(objectsAry[j].extraSpace);
								}
							}
							
							_scene.removeChild( objectsAry[ j ].instance );
							objectsAry[ j  ] = null;
							objectsAry.splice( j, 1 );
							break;
						}
					}					
				}
		...
		private function onDropOfficeItem(e:MouseEvent):void 
		{
			trace('drop office object');
			OBJECT_MOVING = false;
			putToTile(_dragObject);
			removePointer();
			updateOfficeObjectGridXY(_dragObject);
			registerObject(_dragObject , "move" );
			
			_dragObject = null;
			_activeObject = null;
			
			removeEventListener( MouseEvent.MOUSE_MOVE, onObjectMove );
			removeEventListener( MouseEvent.CLICK, onDropOfficeItem );
			//drop drag item here..
		}
		...
		private function onInteractOfficeObject(e:ProxyEvent):void 
		{
			//_dragObject = e.currentTarget as IsoSprite;
			_dragObject = e.target as IsoSprite;
			if ( OBJECT_INTERACTION == "BUY" &&	OBJECT_MOVING ) {				
				trace( "check _dragObject ======>",_dragObject );
			}
		}
	}
}