...
{	 
	...
	public class CharShopWindow extends AbstractWindow
	{
		...
		private var _dialogue:DialogueEvent;		//_mc = new mc_shop1();
		...
		{
			_mc = new mc_shop();		
			_mc._shopxmladdress = _gd.absPath + "characters/shop";//"public/data/charactershop.xml";
			_mc._characterinventory = _gd.absPath + "characters/inventory/";
			_mc._basePath = _gd.absPath;
			
			trace(  _mc._characterinventory  );
			
			// add this _mc.portAddress = "";
			...
			_mc._equipaddress = _gd.absPath + "characters/eqp/";
		...
		private function loadChar( gender:int, setType:String  ):void  //creditsCoin
		{			
			//_mc.gender=1;
			...
			trace("currentCharId", _mc.characterID );
			
			var charShopFont:CharShop_Font = new CharShop_Font();
			_mc.initialize();
			_mc.embedFontStyle( charShopFont );
		...
		private function onReadyCharShop(e:Event):void 
		{
			...
			// fetch item equip and to equip to him
			_mc._inventoryaddress = _gd.absPath + "characters/inventory/"+_charId;
		...
		private function onCharSelected( e:Event ):void //GlobalData
		{
